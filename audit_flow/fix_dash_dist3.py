...
content = re.sub(r"const businessUnitDistribution = \{.*?\}\s*\};\s*openFindings\.forEach\(f => \{\s*// Fake distribution for compilation\s*\}\);\s*const riskDistribution = \{.*?\};", replacement, content, flags=re.DOTALL)
