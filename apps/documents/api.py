...
class DocumentViewSet(viewsets.ModelViewSet):
    ...
    def grant_access(self, request, pk=None):
        ...
        # Team Managers can only request access for Team Members, Auditors, Auditees and Visitors
        if request.user.role == 'TEAM_MANAGER' and target_user.role not in ['TEAM_MEMBER', 'AUDITOR', 'AUDITEE', 'VISITOR']:
            return Response({'error': 'Team Managers can only grant access to Team Members, Auditors, Auditees and Visitors'}, status=status.HTTP_400_BAD_REQUEST)
