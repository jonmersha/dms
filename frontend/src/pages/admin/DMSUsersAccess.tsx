...
export function DMSUsersAccess() {
  ...
  useEffect(() => {
    if (rolesData) {
      const roles = (rolesData.results || rolesData).filter((r: any) => r.name.startsWith('DMS_'));
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['dmsUsers'] });
  ...
  const activeDmsUsers = users.filter((u: any) => u.has_dms_access);
  const nonDmsUsers = users.filter((u: any) => !u.has_dms_access);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <Link to="/admin/dms" className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 mb-2">
            <ArrowLeft size={16} /> Back to DMS Admin
          </Link>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 flex items-center gap-3">
            <Users className="text-blue-600" /> DMS Users Access
          </h1>
          <p className="mt-2 text-sm text-gray-600">
            Grant DMS Subsystem access and assign document-specific roles to users.
          </p>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">DMS Access</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Roles</th>
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {activeDmsUsers.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-4 text-center text-sm text-gray-500">
                    No users currently have DMS access.
                  </td>
                </tr>
              )}
              {activeDmsUsers.map((u: any) => {
      ...
      <Modal isOpen={isModalOpen} onClose={resetForm} title={editingUserId ? "Edit DMS Roles" : "Add Existing User to DMS"}>
            ...
            <div className="flex items-start">
              <div className="flex h-5 items-center">
                <input
                  id="hasDmsAccess"
                  type="checkbox"
                  checked={hasDmsAccess}
                  onChange={(e) => setHasDmsAccess(e.target.checked)}
                  className="h-4 w-4 rounded border-blue-300 text-blue-600 focus:ring-blue-500"
                />
              </div>
              <div className="ml-3 text-sm">
                <label htmlFor="hasDmsAccess" className="font-medium text-blue-900">
                  Allow Subsystem Access
                </label>
                <p className="text-blue-700 text-xs mt-1">
                  Grants the user baseline access to enter the Document Management System.
                </p>
              </div>
            </div>
          </div>
