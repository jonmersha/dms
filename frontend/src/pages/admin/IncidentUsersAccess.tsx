...
export function IncidentUsersAccess() {
  ...
  useEffect(() => {
    if (rolesData) {
      const roles = (rolesData.results || rolesData).filter((r: any) => r.name.startsWith('Incident_'));
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['incidentUsers'] });
  ...
  const activeIncidentUsers = users.filter((u: any) => u.has_irregularity_access);
  const nonIncidentUsers = users.filter((u: any) => !u.has_irregularity_access);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <Link to="/system/branch-audit-admin" className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 mb-2">
            <ArrowLeft size={16} /> Back to Branch Audit Admin
          </Link>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 flex items-center gap-3">
            <Users className="text-blue-600" /> Branch Audit Users Access
          </h1>
          <p className="mt-2 text-sm text-gray-600">
            Grant Branch Audit Subsystem access and assign specific roles to users.
          </p>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Incident Access</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Roles</th>
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {activeIncidentUsers.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-4 text-center text-sm text-gray-500">
                    No users currently have Incident access.
                  </td>
                </tr>
              )}
              {activeIncidentUsers.map((u: any) => {
      ...
      <Modal isOpen={isModalOpen} onClose={resetForm} title={editingUserId ? "Edit Incident Roles" : "Add Existing User to Incident"}>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-md p-4">
            <div className="flex items-start">
              <div className="flex h-5 items-center">
                <input
                  id="hasIrregularityAccess"
                  type="checkbox"
                  checked={hasIrregularityAccess}
                  onChange={(e) => setHasIrregularityAccess(e.target.checked)}
                  className="h-4 w-4 rounded border-blue-300 text-blue-600 focus:ring-blue-500"
                />
              </div>
              <div className="ml-3 text-sm">
                <label htmlFor="hasIrregularityAccess" className="font-medium text-blue-900">
                  Allow Subsystem Access
                </label>
                <p className="text-blue-700 text-xs mt-1">
                  Grants the user baseline access to enter the Incident Management System.
                </p>
              </div>
            </div>
          </div>
