...
import type { AuditUniverseEntity, AnnualPlanItem, Engagement, Finding, ComplianceControl, UserRole } from "../../../types/audit_flow";
...
export default function DashboardKpiView() {
  ...
  const openFindings = findings.filter(f => f.rectificationValidationStatus !== 'Fully Rectified');
  ...
  const rectifiedCount = findings.filter(f => f.rectificationValidationStatus === 'Fully Rectified').length;
  ...
  const overdueFindings = findings.filter(f => {
    if (!f.expectedCompletionDate) return false;
    return new Date(f.expectedCompletionDate) < new Date() && f.rectificationValidationStatus !== 'Fully Rectified';
  }).length;

  const nbeControls = complianceControls.filter(c => c.regulationType === 'NBE');
  ...
  const insaControls = complianceControls.filter(c => c.regulationType === 'INSA');
  ...
  findings.forEach(f => {
    const unit = f.entityName || 'Unassigned';
